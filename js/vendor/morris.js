...
(function() {
  ...
  Morris.Donut = (function(_super) {
    ...
    function Donut(options) {
      ...
      this.el.append('<div class="morris-tooltip"></div>');
      this.raphael = new Raphael(this.el[0]);
      if (this.options.resize) {
        $(window).bind('resize', function(evt) {
          if (_this.timeoutId != null) {
    ...
    Donut.prototype.redraw = function() {
      ...
      for (i = _j = 0, _len1 = _ref1.length; _j < _len1; i = ++_j) {
        ...
        seg = new Morris.DonutSegment(cx, cy, w * 2, w, last+gap, next-gap, this.data[i].color || this.options.colors[idx % this.options.colors.length], this.options.backgroundColor, idx, this.raphael);
        seg.render();
    ...
    Donut.prototype.select = function(idx) {
      ...
      if (typeof window._morris_donut_segment !== 'undefined' && 
          window._morris_donut_segment!==segment)
      {
        window._morris_donut_segment.deselect.call(window._morris_donut_segment);
        window._morris_donut_el.children('.morris-tooltip').removeClass('active');
      }
      window._morris_donut_segment = segment;
      window._morris_donut_el = this.el;

      var middle = segment.calcMiddleArcPoint( segment.inner+25 );
      // var middle = segment.calcArcPoints( segment.inner );
      // segment.x_mid = middle[0];
      // segment.y_mid = middle[1];
      ...
      var $tip = this.el.children('.morris-tooltip').first().html(this.options.formatter(row.value, row));
      $tip.css({left:'', top:'', right:'', bottom:''}).removeClass('left top right bottom');
      $tip
          // .css({left: middle[0]+'px', top: middle[1]+'px'})
          .addClass('active');

      var pos = {};
      var posClasses = '';
      if (middle[0] <= elWidth*0.5)
      {
        pos['right'] = (elWidth - (middle[0]-10))+'px';
        posClasses += 'left ';
      }
      else
      {
        pos['left'] = (middle[0]+10) + 'px';
        posClasses += 'right ';
      }

      if (middle[1] <= elHeight*0.5)
      {
        pos['bottom'] = (elHeight - middle[1])+'px';
        posClasses += 'top ';
      }
      else
      {
  ...
  Morris.DonutSegment = (function(_super) {
    ...
    function DonutSegment(cx, cy, inner, outer, p0, p1, color, backgroundColor, index, raphael) {
      ...
      this.sin_mid = Math.sin(p0 + (p1 - p0)*0.5);
      this.cos_mid = Math.cos(p0 + (p1 - p0)*0.5);
      var middle = this.calcMiddleArcPoint(this.inner );
      this.x_mid = middle[0];
      this.y_mid = middle[1];
      this.is_long = (p1 - p0) > Math.PI ? 1 : 0;
      // this.path = this.calcSegment(this.inner + 3, this.inner + this.outer - 5);
      this.path = this.calcSegment(this.inner + 10, this.inner + this.outer - 12);
      this.selectedPath = this.calcSegment(this.inner + 3, this.inner + this.outer - 5);
      this.hilight = this.calcArc(this.inner);
    }

    DonutSegment.prototype.calcMiddleArcPoint = function(r) {
      // return [this.cx + r * this.cos_mid, this.cy + r * this.sin_mid];
      return [this.cx + r * this.sin_mid, this.cy + r * this.cos_mid];
      // return [this.cx + r * this.cos_p0, this.cy + r * this.sin_p0, ];
    };
    ...
    DonutSegment.prototype.select = function() {
      if (!this.selected) {
        ...
        // var middle = this.calcMiddleArcPoint(this.outer);
        // console.log(middle);
